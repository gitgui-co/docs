...
Learn how to open and manage Git repositories in GitGUI.

## Opening Your First Repository

### From the Welcome Screen

When you launch GitGUI without a repository open, you'll see the welcome screen:

1. Click **Open Repository**
2. Navigate to your Git repository folder
3. Select the folder and click **Open**

GitGUI will load the repository and display the commit graph.
...
GitGUI remembers your recently opened repositories for quick access.
...
When you open a repository, GitGUI:
...
GitGUI works with any standard Git repository:
...
If you select a folder that isn't a Git repository, GitGUI will show an error. Initialize a repository first with `git init` or clone an existing one.
:::

## Working with Multiple Repositories

GitGUI focuses on one repository at a time for simplicity. To work with multiple repositories:

1. Use `Alt+O` to quickly switch between recent repositories
2. Or open multiple GitGUI windows (each can have a different repository)

## Auto-Refresh

GitGUI automatically watches your repository for changes:
...
- GitGUI loads commits incrementally (scroll to load more)
